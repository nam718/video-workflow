...
const newsKw = ['新规','过审','不过审','冲不垮','能否冲垮','能否取代','行业分析','行业报告','最先代替','会代替','取代','冲击','制作成本','高校','专业','招生','招聘','人才缺口','狂招','招人','招工','人才需求','就业','岗位','薪资','月薪','年薪','工资','用工荒','求职'];
...
const filtered = unique.filter(d => {
  ...
  // 营销号名/新闻号特征
  if (/娱乐|八卦|热点|资讯|剪辑号|帮忙$|新闻|报道|日报|晚报|都市报|商报|时报|周刊|周报|早报/.test(nick)) return false;
