...
async function buildSmartScript(items, rankDate, analysisMap) {
  ...
  const prompt = buildNarrationPrompt(clipInfos, rangeText, rankDate, items.length);
...
function buildNarrationPrompt(clipInfos, rangeText, rankDate, totalCount) {
...
12. **outro**（结尾）: 总结本期趋势 + 给创作者的建议 + 引导关注。控制在60字以内。
</structure>
...
function parseAINarration(aiResult, top10, rangeText) {
  ...
  const rankings = segs.filter(s => s.type === 'ranking');
  if (rankings.length < 10) {
    console.warn(`  ⚠️ AI只生成了 ${rankings.length} 个排名段（需要10个）`);
  }

  console.log(`  ✅ AI解说文案生成完成，共 ${segs.length} 段`);
  return segs;
}
...
async function main() {
  ...
  if (useAI) {
    ...
    console.log('\n📝 Step 2: AI智能解说生成');
    segments = await buildSmartScript(ranking.items, ranking.date, analysisMap);
    if (!segments) {
  ...
  try {
    ...
    if (chartResults.length) {
      // 所有图表（黑马+K线+红海蓝海）→ 统一插在 trend 之前
      if (chartResults.length) {
        const trendIdx = segments.findIndex(s => s.type === 'trend');
        const insertAt = trendIdx >= 0 ? trendIdx : segments.length - 1;
        ...
        console.log(`  ✅ 插入 ${chartSegs.length} 个图表段（trend前）`);
      }
    }
  } catch (e) {
